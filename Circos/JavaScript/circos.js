...
function Circos(datasource){
	...
	this.handleMouseDown = function(ctx, location){
		...
		lastAngle = this.getAngle(location);
	...
	this.handleMouseOver = function(ctx, location){
		...
		if(loc.area == 0){
			...
			var offsets = new Array();
			for(var j = 0; j < ds.sweeps.length; j++){
				offsets[j] = 0;
			}
	...
	this.getLocation = function(location){
		//this is set up kind of like a rocket. We start from the center of the earth and start moving out.
		
		var scale = this.ds.scale;
		
		//console.log("this.ds.scale = ".concat(this.ds.scale));
		//console.log("scale = ".concat(scale));
		
		//First, We launch out rocket and see how high it travels
		var realLocation = { //location with center @ 0
			x: scale * (location.x - this.ds.center.x),
			y: scale* (location.y - this.ds.center.y)
		};
		//console.log("(".concat(realLocation.x).concat(", ").concat(realLocation.y).concat(")"));
		var dist = Math.sqrt(realLocation.x * realLocation.x + realLocation.y * realLocation.y); //pythagorean theorum
	...
	this.pinpoint = function(location){
